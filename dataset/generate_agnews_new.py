...
def generate_ag_news(dir_path, num_clients, num_classes, niid, balance, partition, alpha, few_shot, n_shot, pfl):
    ...
    train_images = [x for x in train_dataset]
    test_images = [x for x in test_dataset]
    ...
    if pfl:
        X, _, statistic = separate_data_pfl((dataset_image,), num_clients, num_classes,  
        ...
        if not niid:  # iid
            train_data, test_data, statistic, statistic_test = separate_data_few_shot_iid((dataset_image,), 
                                                        num_clients, num_classes, n_shot)
        else:  # pat
            train_data, test_data, statistic, statistic_test = separate_data_few_shot_pat_non_iid((dataset_image,), 
                                                        num_clients, num_classes, n_shot)
        
    else:
        train_data, test_data, statistic, statistic_test = separate_data((dataset_image,), num_clients, num_classes,  
                                    niid, balance, partition, alpha, class_per_client=4)
