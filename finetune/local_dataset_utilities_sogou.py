...
def download_dataset():
    # The Sogou News dataset will be downloaded using the datasets library, so this function can be removed
    pass
...
def tokenization_sogou():
    sogou_news_dataset = load_dataset(
        "csv",
        data_files={
    ...
    sogou_news_tokenized = sogou_news_dataset.map(tokenize_text, batched=True, batch_size=None)
    sogou_news_tokenized.set_format("torch", columns=["input_ids", "attention_mask", "label"])
    os.environ["TOKENIZERS_PARALLELISM"] = "false"
    return sogou_news_tokenized

def setup_dataloaders_sogou(sogou_news_tokenized):
    train_dataset = SogouNewsDataset(sogou_news_tokenized, partition_key="train")
    val_dataset = SogouNewsDataset(sogou_news_tokenized, partition_key="validation")
    test_dataset = SogouNewsDataset(sogou_news_tokenized, partition_key="test")
...
sogou_news_tokenized = tokenization()
train_loader, val_loader, test_loader = setup_dataloaders(sogou_news_tokenized)
