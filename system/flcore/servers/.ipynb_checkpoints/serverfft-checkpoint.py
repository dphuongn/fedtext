...
import copy
import random
from flcore.clients.clientfft import clientFFT
...
class FedFft(Server):
    def __init__(self, args, times):
        ...
        self.global_model = copy.deepcopy(self.model_object.model)    # bert model
    ...
    def send_models(self):
        # sending the whole model
        assert (len(self.clients) > 0)

        for client in self.clients:
            start_time = time.time()
            
            global_params = self.global_model
    ...
    def receive_models(self):
        # Receive CLIP model from each client
